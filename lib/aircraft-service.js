...
function refreshAircraftDataInBackground() {
    if (runtimeState.refreshPromise) return;
    runtimeState.refreshPromise = refreshAircraftData()
        .catch(error => console.warn('Background aircraft refresh failed:', error.message))
...
async function getAircraftData() {
    try {
        ...
        if (runtimeState.cachedData && age < MAX_STALE_AGE) {
            ...
            refreshAircraftDataInBackground();
            return response(200, runtimeState.cachedData, 'STALE-REFRESHING');
        }
        console.log('No cache available, fetching from OpenSky...');
        const refreshed = await refreshAircraftData();
        if (refreshed) return response(200, runtimeState.cachedData, 'MISS');
...
module.exports = { classifyApproachDirection, distanceInKm, enrichGvaArrivals, getAircraftData, normalizeOpenSkyData, pruneEnrichmentCache };
