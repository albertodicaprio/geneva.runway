...
async function fetchOpenSkyStates({ forceTokenRefresh = false } = {}) {
    ...
    return fetchWithTimeout(url, { method: 'GET', headers: { Accept: 'application/json', Authorization: `Bearer ${token}` } }, OPENSKY_TIMEOUT);
}
    ...
    try {
        ...
        if (runtimeState.cachedData && age < MAX_STALE_AGE) {
            if (age < CACHE_DURATION) {
                ...
                return response(200, projectAircraftData(runtimeState.cachedData), 'HIT');
            }
            ...
            return response(200, projectAircraftData(runtimeState.cachedData), 'STALE-REFRESHING');
        }
        console.log('No cache available, fetching from OpenSky...');
        const refreshed = await refreshAircraftDataOnce();
        if (refreshed) return response(200, projectAircraftData(runtimeState.cachedData), 'MISS');
        ...
        if (runtimeState.cachedData) return response(200, projectAircraftData(runtimeState.cachedData), 'STALE-ERROR');
        return { status: 500, body: { error: 'Failed to fetch aircraft data' }, headers: {} };
    }
}

module.exports = { classifyApproachDirection, distanceInKm, enrichGvaArrivals, fetchWithTimeout, getAircraftData, normalizeOpenSkyData, projectAircraftData, pruneEnrichmentCache, refreshAircraftDataOnce };
