...
const GENEVA_AIRPORT = { latitude: 46.2381, longitude: 6.1093 };
...
async function loadCachedAircraftData() {
    ...
    try {
        ...
        if (cache && cache.version === CACHE_VERSION && cache.cachedData && Number.isFinite(cache.lastFetchTime)) {
...
function distanceInKm(latitude, longitude) {
    ...
    const a = Math.sin(latitudeDelta / 2) ** 2 +
        Math.cos(toRadians(GENEVA_AIRPORT.latitude)) * Math.cos(toRadians(latitude)) *
        Math.sin(longitudeDelta / 2) ** 2;
    return earthRadiusKm * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
}

function normalizeAircraftData(data) {
    ...
    return {
        updatedAt: data.time || null,
        airport: { icao: 'LSGG', ...GENEVA_AIRPORT },
        ...
        if (runtimeState.cachedData && age < MAX_STALE_AGE) {
            ...
            try {
                ...
                if (statesResponse.ok) {
                    const data = normalizeAircraftData(await statesResponse.json());
...
module.exports = { getAircraftData, distanceInKm, normalizeAircraftData };
