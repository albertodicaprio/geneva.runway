...
const { createFlightHistory } = require('./flight-history');
const { addArrivalTracks, addGeneralTraffic, normalizeOpenSkyData, projectAircraftData } = require('./traffic');

const CACHE_DURATION = 30000;
const NEXT_SNAPSHOT_WAIT = 25000; // Below Caddy's 30-second response timeout.
...
const AIRCRAFT_HISTORY_DIR = process.env.AIRCRAFT_HISTORY_DIR || path.join(path.dirname(AIRCRAFT_CACHE_FILE), 'flight-history');
const fileHistoryStore = createFlightHistory({ directory: AIRCRAFT_HISTORY_DIR });
...
function createAircraftService({ fetch: fetchImpl = globalThis.fetch, clock = Date.now,
    cacheStore = fileCacheStore, historyStore = fileHistoryStore,
    credentials, schedule = setInterval, waitTimeoutMs = NEXT_SNAPSHOT_WAIT } = {}) {
    const state = {
        cachedData: null, lastFetchTime: 0, refreshPromise: null,
        nextRefreshAt: 0, refreshInterval: null,
        loadPromise: null, loaded: false, refreshWaiters: new Set()
    };
    ...
    function cachedResponseBody() {
        return {
            ...
            cacheUpdatedAtMs: state.lastFetchTime
        };
    }

    function waitForNewerSnapshot(after) {
        if (state.lastFetchTime > after) return Promise.resolve();
        return new Promise(resolve => {
            let timer;
            const finish = () => {
                clearTimeout(timer);
                state.refreshWaiters.delete(finish);
                resolve();
            };
            state.refreshWaiters.add(finish);
            timer = setTimeout(finish, waitTimeoutMs);
        });
    }
    ...
    async function saveCachedAircraftData(data) {
        ...
        for (const notify of state.refreshWaiters) notify();
    }
    ...
    async function refreshAircraftData() {
        ...
        try {
            await historyStore.recordSnapshot(data);
        } catch (error) {
            console.warn('Failed to write flight history:', error.message);
        }
    ...
    async function getAircraftData({ after = null } = {}) {
        try {
            await loadCachedAircraftData();
            if (Number.isFinite(after) && state.cachedData && state.lastFetchTime <= after) {
                if (!isAircraftRefreshPaused(clock()) && clock() - state.lastFetchTime >= CACHE_DURATION)
                    void refreshAircraftDataOnce();
                await waitForNewerSnapshot(after);
            }
    ...
    function getFlightHistory(days, offset = 0) {
        return historyStore.readDays(days, clock(), offset);
    }

    return { getAircraftData, getFlightHistory, refreshAircraftDataOnce, startAircraftRefreshScheduler };
}
...
module.exports = { createAircraftService, isAircraftRefreshPaused,
    ...
    getFlightHistory: defaultService.getFlightHistory,
