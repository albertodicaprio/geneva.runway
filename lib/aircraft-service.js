...
async function getAdsbdbCached(kind, key, url, duration, shouldPersist = () => true) {
    ...
    try {
        ...
        if (apiResponse.ok || apiResponse.status === 404) {
            const entry = { fetchedAt: Date.now(), notFound: apiResponse.status === 404, data: apiResponse.data };
...
function addGeneralTraffic(arrivals, normalizedData, previousData = runtimeState.cachedData) {
    ...
    const timestamp = Number.isFinite(normalizedData.updatedAt) ? normalizedData.updatedAt : Math.floor(Date.now() / 1000);
    return {
        ...arrivals,
...
async function refreshAircraftData() {
    ...
    const data = await enrichGeneralTraffic(addGeneralTraffic(addArrivalTracks(await enrichGvaArrivals(normalizedData)), normalizedData));
...
function refreshAircraftDataOnce() {
    ...
    if (Date.now() < runtimeState.nextRefreshAt) return Promise.resolve(false);
    // Limit attempts, including failures, across both polling and the scheduler.
    runtimeState.nextRefreshAt = Date.now() + CACHE_DURATION;
...
function startAircraftRefreshScheduler() {
    if (runtimeState.refreshInterval) return;

    const refresh = () => {
        if (!isAircraftRefreshPaused()) void refreshAircraftDataOnce();
    };
    refresh();
    runtimeState.refreshInterval = setInterval(refresh, CACHE_DURATION);
    runtimeState.refreshInterval.unref();
}

async function getAircraftData() {
    try {
        await loadCachedAircraftData();
        if (isAircraftRefreshPaused()) {
            if (runtimeState.cachedData) return response(200, cachedResponseBody(), 'PAUSED');
            return {
                status: 503,
...
module.exports = { enrichGeneralTraffic, addGeneralTraffic, addArrivalTracks, classifyApproachDirection, distanceInKm, enrichGvaArrivals, fetchWithTimeout, getAircraftData, isAircraftRefreshPaused, isWithinSearchBounds, normalizeOpenSkyData, projectAircraftData, pruneEnrichmentCache, refreshAircraftDataOnce, startAircraftRefreshScheduler };
