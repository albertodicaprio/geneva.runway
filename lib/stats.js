...
function summarizeFlights(flights, days) {
    const landing = flights.filter(flight => flight.category === 'arrival');
    const general = flights.filter(flight => flight.category !== 'arrival');
    return {
        days,
        landing: summarizeGroup(landing),
        general: {
            ...summarizeGroup(general),
            departures: general.filter(flight => flight.category === 'departure').length,
            other: general.filter(flight => flight.category !== 'departure').length
        }
    };
}
